...
class T3A(Algorithm):
    ...
    def __init__(self, featurizer, classifier, args, filter_K = 100, num_classes = 5):
        super(T3A, self).__init__(args)
    ...
    def forward(self, x, adapt=False):
        ...
        if adapt:
            ...
            self.supports = torch.concat([self.supports, z])
            self.labels = torch.concat([self.labels, yhat])
            self.ent = torch.concat([self.ent, ent])

        supports, labels = self.select_supports()
        supports = torch.nn.functional.normalize(supports, dim=1)
        weights = (supports.T @ (labels))
        z = z.to(weights.device)
    ...
    def select_supports(self):
        ...
        for i in range(self.num_classes):
            ...
            # print('for',indices1[y_hat == i][indices2][:filter_K].shape)
